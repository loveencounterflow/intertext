(function() {
  'use strict';
  var CND, DATOM, HtmlParser, alert, assign, badge, debug, echo, find_next_tag, help, info, isa, jr, lets, log, new_datom, rpr, select, type_of, types, urge, validate, warn, whisper,
    indexOf = [].indexOf;
  ...
  //-----------------------------------------------------------------------------------------------------------
  this._analyze_mkts_compact_syntax = function(datoms) {
    var $key, attribute, attributes, avalue, d, i, idx, j, len, len1, ref, update;
    for (idx = i = 0, len = datoms.length; i < len; idx = ++i) {
      ...
      for (j = 0, len1 = ref.length; j < len1; j++) {
        ...
        if (attribute[0] === '#') {
          ...
          (update.clasz != null ? update.clasz : update.clasz = []).push(avalue);
        }
      }
      if (update.clasz != null) {
        // debug '^3388^', attributes
        update.clasz = update.clasz.join(' ');
      }
      datoms[idx] = lets(d, function(d) {
        return assign(d, update);
      });
    }
    return datoms;
  };
  ...
  this.mkts_html_as_datoms = function(text) {
    ...
    while (true) {
      ...
      tags = this._analyze_mkts_compact_syntax(this.html_as_datoms(text.slice(idx_0, +idx_1 + 1 || 9e9)));
      if (text[idx_1 - 1] === '/') {
