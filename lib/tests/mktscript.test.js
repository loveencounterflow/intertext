(function() {
  ...
  */
  ...
  //-----------------------------------------------------------------------------------------------------------
  this["HTML.mkts_html_as_datoms (compact syntax)"] = async function(T, done) {
    var error, i, key, len, matcher, probe, probes_and_matchers, value;
    probes_and_matchers = [
      [
        // [ '<columns =2 =3>' ]
        "<div>",
    ...
    for (key in HTML) {
      value = HTML[key];
      debug('^3443^', key, type_of(value));
    }
    for (i = 0, len = probes_and_matchers.length; i < len; i++) {
      [probe, matcher, error] = probes_and_matchers[i];
      await T.perform(probe, matcher, error, function() {
        return new Promise(function(resolve, reject) {
          return resolve(HTML.mkts_html_as_datoms(probe));
        });
      });
    }
    //.........................................................................................................
...
<h1#c4443><strong.myclass>CHAPTER VI.</strong> <name ref=hd553>Humpty Dumpty</h1>`;
    ...
    ref = datoms = HTML.mkts_html_as_datoms(buffer);
    // debug '^80009^', buffer
