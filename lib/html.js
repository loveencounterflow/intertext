(function() {
  ...
  //-----------------------------------------------------------------------------------------------------------
  this.datoms_as_nlhtml = function(...ds) {
    var R, d, html, i, len, ref, sigil, tagname;
    R = '';
    ref = ds.flat(2e308);
    for (i = 0, len = ref.length; i < len; i++) {
      d = ref[i];
      html = this._datom_as_html(d);
      sigil = d.$key[0];
      tagname = d.$key.slice(1);
      if (sigil === '<' && isa._intertext_html_block_level_tagname(tagname)) {
        R += '\n';
      }
      R += html;
    }
  ...
  this.datoms_as_html = function(...ds) {
    ...
    return ((function() {
      ...
      for (i = 0, len = ref.length; i < len; i++) {
        d = ref[i];
        results.push(this._datom_as_html(d));
      }
      return results;
    }).call(this)).join('');
  };
  ...
  //-----------------------------------------------------------------------------------------------------------
  this._datom_as_html = function(d) {
  ...
  this.$datom_as_html = function() {
    ...
    return $((d, send) => {
      send(this._datom_as_html(d));
      return null;
    });
  };

  //-----------------------------------------------------------------------------------------------------------
  this.$datoms_as_html = function() {
    var $;
    ({$} = (require('steampipes')).export());
    return $((d, send) => {
      send(this.datoms_as_html(d));
      return null;
    });
  };
  ...
  //-----------------------------------------------------------------------------------------------------------
  this.html_as_datoms = function(html) {
